...
function validateEmailSignup(req, res, next) {
    ...
    const { error, value } = emailSignupFilter.validate(req.body, validationOptions);
    if (error) {
...
function validateEmailLogin(req, res, next) {
    ...
    const { error, value } = emailLoginFilter.validate(req.body, validationOptions);
    if (error) {
        res.status(STATUS_CODES.BAD_REQUEST);
        next(`Validation error: ${error.details.map(x => x.message).join(', ')}`);
    }
...
function validateGoogleSignin(req, res, next) {
    ...
    const { error, value } = googleLoginFilter.validate(req.body, validationOptions);
    if (error) {
        res.status(STATUS_CODES.BAD_REQUEST);
        next(`Validation error: ${error.details.map(x => x.message).join(', ')}`);
    }
...
function validateFacebookSignin(req, res, next) {
    ...
    const { error, value } = facebookLoginFilter.validate(req.body, validationOptions);
    if (error) {
        res.status(STATUS_CODES.BAD_REQUEST);
        next(`Validation error: ${error.details.map(x => x.message).join(', ')}`);
    }
